...
неподдерживаемыми типами (список поддерживаемых типов см. ниже описание параметра type). Также
у основных функций должны поддерживаться методы ToString, возвращающие строковое представление
функции.
Очередность:
...
находит корень уравнения f(x) = 0 методом градиентного спуска. Проверять сходимость в коде не надо.
Число итераций градиентного спуска должно быть параметром функции.
...
(6) Вопросы по заданию следует направлять В. Балашову в Телеграм (@slonoul).
