...
std::vector<uint32_t> loadJobDurationsFromCsv(const std::string& filename) {
    ...
    while (std::getline(file, line)) {
        ...
        // Восстанавливаем числовой идентификатор работы из Job_ID и помещаем длительность
        // в соответствующий индекс вектора (если идентификатор не найден, добавляем в конец).
